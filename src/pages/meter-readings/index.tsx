...
function MeterReadingListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MeterReadingInterface[]>(
    ...
    () =>
      getMeterReadings({
        relations: ['consumer', 'user', 'water_bill.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>reading</Th>
                  <Th>reading_date</Th>
                  {hasAccess('consumer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>consumer</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('consumer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/consumers/view/${record.consumer?.id}`}>
